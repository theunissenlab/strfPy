...
def generate_laguerre_features(pair, feature_key, laguerre_args=np.zeros((2,3)), nLaguerrePoints=300, nLaguerre=5):
    """
    ...
    for iEventType in range(nEventsTypes):
        ...
        for iLaguerre in range(nLaguerre):
            lag_start_ind = iLaguerre*nFeaturesPerEventType*nEventsTypes + iEventType*nFeaturesPerEventType
            y = laguerre(x_t, amp=laguerre_amp, tau=laguerre_tau, alpha=laguerre_alpha, xorder=iLaguerre)
            y = y / np.sqrt(np.sum(y**2))
            laguerre_mat[lag_start_ind:lag_start_ind+nFeaturesPerEventType,:] = y[np.newaxis,:]
...
def get_prediction_r2(pair, ridge, feature, laguerre_args, ridge_conv_filter, nPoints, nLaguerre=5):
    y = pair['resp']['psth_smooth']
    y_pred = generate_prediction(pair, ridge, feature, laguerre_args, ridge_conv_filter, nPoints, nLaguerre)
    return np.corrcoef(y, y_pred)[0,1]**2
...
def generate_prediction(pair, ridge, feature, laguerre_args, ridge_conv_filter, nPoints=200, nLaguerre=5):
...
def generate_pred_score(pair, ridge, feature, laguerre_args, ridge_conv_filter, nPoints=200, nLaguerre=5):
    x = generate_laguerre_features(pair, 'pca_%s'%feature,  laguerre_args, nLaguerrePoints=nPoints, nLaguerre=nLaguerre)
...
def preprocess_srData(srData, plot=False, respChunkLen=150, segmentBuffer=25,tdelta=0):
    ...
    for iSet in range(pairCount) :
        ...
        events['onoff_feature'] =  np.vstack([np.ones(len(peakInd), dtype=int), np.zeros(len(peakInd), dtype=int)]).T # these are event 0
        events['index'].extend(troughInd)
        events['onoff_feature'] = np.concatenate( [events['onoff_feature'],np.vstack([np.zeros(len(troughInd), dtype=int), np.ones(len(troughInd), dtype=int)]).T]) # these are event 1
        events['onoff_feature_values'] = np.concatenate([peakVals['peak_heights'] / peakVals['peak_heights'].max(),troughVals['peak_heights'] / troughVals['peak_heights'].max()])
        # sort the features by index
        events['onoff_feature'] = events['onoff_feature'][np.argsort(events['index'])]
        events['onoff_feature_values'] = events['onoff_feature_values'][np.argsort(events['index'])]
        ...
        events['spect_windows_nfeats'] = events['spect_windows'].shape[1]*events['spect_windows'].shape[2]
        spect_feats = events['spect_windows'].reshape((len(events['index']), events['spect_windows_nfeats']))
        ...
        events['log_spect_windows_nfeats'] = events['log_spect_windows'].shape[1]*events['log_spect_windows'].shape[2]
        ...
        for i in range(len(events['index'])):
            ...
            window=.05
            Norm=True
            wf, wt, mps_powAvg = mps(spect, srData['datasets'][iSet]['stim']['tfrep']['f'], np.arange(respChunkLen)/ampfs, window=window, Norm=Norm)
        ...
        events['mps_windows_nfeats'] = events['mps_windows'].shape[1]*events['mps_windows'].shape[2]
    ...
    all_mps = np.concatenate([srData['datasets'][iSet]['events']['mps_windows'] for iSet in range(pairCount)], axis=0)
    mean_mps = np.mean(all_mps, axis=0)
    for iSet in range(pairCount):
        srData['datasets'][iSet]['events']['mps_windows'] = srData['datasets'][iSet]['events']['mps_windows'] - mean_mps


# fitting funcitons


def fit_seg_model(srData, nLaguerre, nPoints, event_types, feature, pair_train_set=None, pca=None):
    """
...
def process_unit(srData, feature='log_spect_windows', nPoints=200, mult_values=False, plot=False, nLaguerre = 5, pca=None, do_test_set=False):
    ...
    pca, ridge, ridge_conv_filter, laguerre_args = fit_seg_model(srData, nLaguerre, nPoints, event_types, feature, pair_train_set, mult_values=mult_values, pca=pca)
    ...
    all_r2_train = [get_prediction_r2(srData['datasets'][iSet], ridge, feature, laguerre_args, ridge_conv_filter, nPoints, nLaguerre) for iSet in pair_train_set]
    all_r2_test = [get_prediction_r2(srData['datasets'][iSet], ridge, feature, laguerre_args, ridge_conv_filter, nPoints, nLaguerre) for iSet in pair_test_set]
    ...
    return srData, pca, ridge, ridge_conv_filter, laguerre_args, [np.mean(simple_r2_train), np.mean(simple_r2_test), np.mean(all_r2_train), np.mean(all_r2_test)]#, R2Ceil
